...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Next.js (server-side)**: API routes (`/pages/api/` or `/app/api/`) missing authentication checks, `getServerSideProps` / server components leaking secrets into client bundles, `dangerouslySetInnerHTML` without sanitization, open redirect via `router.push` or `res.redirect` with user-controlled URLs, missing CSRF protection on mutating API route handlers (POST/PUT/DELETE), `next.config.js` rewrites/redirects with patterns that enable SSRF, environment variables prefixed `NEXT_PUBLIC_` accidentally exposing secrets to the browser, server actions without proper authentication/authorization checks, Edge Runtime middleware missing auth for protected paths, `next-auth` misconfiguration exposing session tokens or weak JWT secrets
