...
export default function Transfers() {
  ...
  const handleAddRow = async (newData: Transfer) => {
    ...
    try {
      // Generate UUIDs from server (like transaction page does)
      const [guidSourceResponse, guidDestinationResponse] = await Promise.all([
      ...
      if (!guidSourceResponse.ok || !guidDestinationResponse.ok) {
        throw new Error("Failed to generate UUIDs");
      }

      const { uuid: guidSource } = await guidSourceResponse.json();
      const { uuid: guidDestination } = await guidDestinationResponse.json();
