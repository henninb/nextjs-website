import { DataValidator, InputSanitizer, FINANCIAL_LIMITS } from '../utils/validation';
...
describe('Input Validation and Sanitization', () => {
  describe('DataValidator.validateUser', () => {
    ...
    it('should reject weak password', () => {
      ...
      expect(result.errors).toBeDefined();
      if (result.errors) {
        const hasPasswordError = result.errors.some(err => err.field === 'password');
  ...
  describe('DataValidator.validatePayment', () => {
    it('should reject payment with same source and destination', () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors?.some(err => err.code === 'SAME_ACCOUNT_ERROR')).toBe(true);
    });
  ...
  describe('InputSanitizer', () => {
    ...
    it('should sanitize descriptions', () => {
      ...
      const sanitized = InputSanitizer.sanitizeDescription(maliciousDescription);
      expect(sanitized).not.toContain('<script>');
  ...
  describe('Financial Boundary Checks', () => {
    it('should detect suspicious amounts', () => {
      ...
      const result = DataValidator.validateFinancialBoundaries(suspiciousTransaction);
      expect(result.success).toBe(false);
      expect(result.errors?.some(err => err.code === 'SUSPICIOUS_AMOUNT')).toBe(true);
    });
    ...
    it('should reject dates too far in the past', () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors?.some(err => err.code === 'DATE_TOO_OLD')).toBe(true);
    });
    ...
    it('should reject dates too far in the future', () => {
      ...
      const result = DataValidator.validateFinancialBoundaries(futureTransaction);
      expect(result.success).toBe(false);
      expect(result.errors?.some(err => err.code === 'DATE_TOO_FUTURE')).toBe(true);
    });
  });
});