...
describe("useFinanceValidation Hook", () => {
  describe("Amount Validation", () => {
    it("validates valid amounts", () => {
      ...
      expect(result.current.validateAmount("100")).toEqual({ isValid: true });
      expect(result.current.validateAmount(100.5)).toEqual({ isValid: true });
      expect(result.current.validateAmount("100.50")).toEqual({
        isValid: true,
      });
      expect(result.current.validateAmount(-100)).toEqual({ isValid: true });
  ...
  describe("Category Validation", () => {
    it("validates valid categories", () => {
      ...
      expect(result.current.validateCategory("Transportation")).toEqual({
        isValid: true,
      });
      expect(result.current.validateCategory("Food & Dining")).toEqual({
        isValid: true,
      });
      expect(result.current.validateCategory("Gas-Station")).toEqual({
        isValid: true,
      });
      expect(result.current.validateCategory("ATM (Withdrawal)")).toEqual({
        isValid: true,
      });
    });
  ...
  describe("Description Validation", () => {
    it("validates valid descriptions", () => {
      ...
      expect(result.current.validateDescription("Grocery shopping")).toEqual({
        isValid: true,
      });
      expect(
        result.current.validateDescription("ATM withdrawal at Bank"),
      ).toEqual({ isValid: true });
      expect(
        result.current.validateDescription("Payment for services @company"),
      ).toEqual({ isValid: true });
    });
  ...
  describe("Account Name Validation", () => {
    it("validates valid account names", () => {
      ...
      expect(result.current.validateAccountName("Chase Checking")).toEqual({
        isValid: true,
      });
      expect(result.current.validateAccountName("Savings-Account_1")).toEqual({
        isValid: true,
      });
      expect(result.current.validateAccountName("Credit Card (Main)")).toEqual({
        isValid: true,
      });
    });
  ...
  describe("Date Validation", () => {
    it("validates valid dates", () => {
      ...
      expect(result.current.validateDate("2024-01-01")).toEqual({
        isValid: true,
      });
      expect(result.current.validateDate("2023-12-31")).toEqual({
        isValid: true,
      });
  ...
  describe("Edge Cases", () => {
    ...
    it("handles very large and very small numbers", () => {
      ...
      expect(result.current.validateAmount(-0.01)).toEqual({ isValid: true });
      expect(result.current.validateAmount(999999.99)).toEqual({
        isValid: true,
      });
      expect(result.current.validateAmount(-999999.99)).toEqual({
        isValid: true,
