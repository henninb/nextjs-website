...
function inferredYear(month: number, day: number): number {
  ...
  return candidate > thirtyDaysAhead
    ? now.getFullYear() - 1
    : now.getFullYear();
}
...
describe("parseTransactionPaste", () => {
  ...
  describe("return value shape", () => {
    ...
    it("should return an empty array when no transaction header is found", () => {
      expect(
        parseTransactionPaste("just some random\ntext with no blocks"),
      ).toEqual([]);
    });
  ...
  describe("Format A — labeled row header", () => {
    ...
    it("should parse the date from a 4-digit year", () => {
      const [row] = parseTransactionPaste(
        blockA(1, "04/16/2026", "FOOD MART", "#...9999", "$60.21"),
      );
      expect(row.date!.getFullYear()).toBe(2026);
      expect(row.date!.getDate()).toBe(16);
    });
    ...
    it("should preserve descriptions with special characters (apostrophe, numbers)", () => {
      const [row] = parseTransactionPaste(
        blockA(1, "04/16/26", "BURGER JOINT 'S F99999", "#...9999", "$1.07"),
      );
  ...
  describe("Format B — plain double-date header", () => {
    ...
    it("should use the first (posted) date when two dates appear on the header", () => {
      const [row] = parseTransactionPaste(
        "04/13/26    04/15/26    SOME MERCHANT\n#REF\n$25.00",
      );
    ...
    it("should ignore the alphanumeric reference line", () => {
      const [row] = parseTransactionPaste(
        blockB(
          "04/13/26",
          "JOHN'S DISCOUNT FOODS RIVERSIDE XX",
          "#9876543FAABBCC002",
          "$44.55    $12,295.67",
        ),
      );
    ...
    it("should extract only the first (charge) amount and ignore the running balance", () => {
      const [row] = parseTransactionPaste(
        blockB(
          "04/13/26",
          "BRAKES PLUS 112233 SPRINGFIELD XX",
          "#REF",
          "$41.03    $12,245.27",
        ),
      );
  ...
  describe("Format C — mobile/app card view", () => {
    ...
    it("should infer the previous year when the date would be > 30 days in the future", () => {
      ...
      const [row] = parseTransactionPaste(
        blockC(dateStr, "FUTURE MERCHANT", "$9.99"),
      );
    ...
    it("should skip the cardholder initials line (MH)", () => {
      const [row] = parseTransactionPaste(
        blockC("Apr 16", "FOOD MART", "$1.89", "MH"),
      );
      expect(row.amount).toBe(1.89);
      expect(row.parseErrors).toHaveLength(0);
    });
    ...
    it("should skip the cardholder initials line (LH)", () => {
      const [row] = parseTransactionPaste(
        blockC("Apr 12", "GENERAL STORE 011445SPRINGFIELD XX", "$5.40", "LH"),
      );
      expect(row.amount).toBe(5.4);
      expect(row.parseErrors).toHaveLength(0);
    });
    ...
    it("should parse multiple consecutive blocks", () => {
      ...
      expect(rows[1]).toMatchObject({
        description: "GENERAL STORE T9999",
        amount: 17.28,
      });
      expect(rows[2]).toMatchObject({
        description: "SAM'S CORNER MART #5 RIVERSIDE XX",
        amount: 44.45,
      });
    });
    ...
    it("should handle all months of the year", () => {
      ...
      for (const [abbrev, monthIdx] of months) {
        const [row] = parseTransactionPaste(
          blockC(`${abbrev} 15`, "MERCHANT", "$10.00"),
        );
  ...
  describe("Format D — tabular single-line export", () => {
    ...
    it("should parse a Return (negative amount)", () => {
      const [row] = parseTransactionPaste(
        blockD(
          "03-09-2026",
          "GENERAL STORE.COM 800-555- CREDIT",
          "-$2.25",
          "Return",
          "",
        ),
    ...
    it("should ignore the card suffix (**XXXX) and transaction type field", () => {
      const [row] = parseTransactionPaste(
        blockD(
          "03-10-2026",
          "GENERAL STORE 9999 SPRINGFIELD XX",
          "$2.51",
          "Sale",
          "**9999",
        ),
    ...
    it("should parse multiple consecutive Format D blocks", () => {
      ...
      expect(rows[0]).toMatchObject({
        description: "GENERAL STORE 9999 SPRINGFIELD XX",
        amount: 23.69,
      });
      expect(rows[1]).toMatchObject({
        description: "GENERAL STORE 9999 SPRINGFIELD XX",
      ...
      expect(rows[2]).toMatchObject({
        description: "GENERAL STORE.COM 800-555- CREDIT",
        amount: -2.25,
      });
    ...
    it("should flag a row when the data line is missing", () => {
      ...
      expect(
        row.parseErrors.some((e) => e.toLowerCase().includes("amount")),
      ).toBe(true);
    });
...
GENERAL STORE 9999 SPRINGFIELD XX    Sale    **9999    $18.17    `;
  ...
  describe("Format E — Citi-style card view (MMM DD, YYYY)", () => {
    ...
    it("should take the first dollar amount and ignore the running balance", () => {
      ...
      expect(row.amount).toBe(122.84);
      expect(row.amount).not.toBe(5678.9);
    });
    ...
    it("should parse multiple consecutive blocks", () => {
      ...
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
      expect(rows[0]).toMatchObject({
        description: "WAREHOUSE CLUB #1234 SPRINGFIELD XX",
        amount: 122.84,
      });
      expect(rows[1]).toMatchObject({
        description: "WAREHOUSE CLUB #1234 SPRINGFIELD XX",
      ...
      expect(rows[2]).toMatchObject({
        description: "7890 PARKWAY AVE CENTERVILLE XX",
        amount: 15.98,
      });
    });
...
$5,551.83`;
  ...
  describe("Format F — labeled row header with inline amount", () => {
    it("should parse a single transaction with inline amount", () => {
      const [row] = parseTransactionPaste(
        blockF(
          2,
          "04/27/26",
          "ONLINE TRANSFER FROM SAVINGS REF #XXXXXXXXXX",
          "$26.78",
        ),
      );
    ...
    it("should parse multiple consecutive Format F blocks", () => {
      const raw = [
        ...
        blockF(
          3,
          "04/27/26",
          "ONLINE TRANSFER FROM CHECKING REF #YYYYYYYYYY",
          "$20.09",
        ),
      ].join("\n");
      const rows = parseTransactionPaste(raw);
      ...
      expect(rows[0]).toMatchObject({
        ...
        notes: "FROM SAVINGS REF #XXXXXXXXXX",
        amount: 26.78,
      });
      expect(rows[1]).toMatchObject({
        description: "ONLINE TRANSFER",
        notes: "FROM CHECKING REF #YYYYYYYYYY",
        amount: 20.09,
      });
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
    });
    ...
    it("should not confuse Format F (inline $) with Format A (follow-on $ line)", () => {
      ...
      expect(rows).toHaveLength(2);
      expect(rows[0]).toMatchObject({
        description: "INLINE AMOUNT TRANSFER",
        amount: 26.78,
      });
      expect(rows[1]).toMatchObject({
        description: "FOLLOW ON AMOUNT",
        amount: 60.21,
      });
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
    });
  ...
  describe("Format G — single-date single-line debit", () => {
    it("should parse a single transaction", () => {
      const [row] = parseTransactionPaste(
        blockG(
          "04/24/26",
          "ONLINE TRANSFER FROM SAVINGS REF #XXXXXXXXXX PLATINUM SAVINGS STORE A",
          "$21.76",
        ),
      );
      expect(row.description).toBe("ONLINE TRANSFER");
      expect(row.notes).toBe(
        "FROM SAVINGS REF #XXXXXXXXXX PLATINUM SAVINGS STORE A",
      );
    ...
    it("should parse the date correctly", () => {
      const [row] = parseTransactionPaste(
        blockG("04/24/26", "BANK REWARDS CREDIT", "$164.16"),
      );
      expect(row.date).toBeInstanceOf(Date);
      expect(row.date!.getFullYear()).toBe(2026);
      expect(row.date!.getMonth()).toBe(3); // April
    ...
    it("should parse multiple consecutive Format G blocks", () => {
      const raw = [
        ...
        blockG(
          "04/24/26",
          "ONLINE TRANSFER FROM CHECKING REF #YYYYYYYYYY WAY2SAVE SAVINGS STORE B",
          "$10.88",
        ),
      ...
      expect(rows[0]).toMatchObject({
        description: "ONLINE TRANSFER",
        notes: "FROM SAVINGS REF #XXXXXXXXXX PLATINUM SAVINGS STORE A",
        amount: 21.76,
      });
      expect(rows[1]).toMatchObject({
        description: "ONLINE TRANSFER",
        notes: "FROM CHECKING REF #YYYYYYYYYY WAY2SAVE SAVINGS STORE B",
        amount: 10.88,
      });
      expect(rows[2]).toMatchObject({
        description: "BANK REWARDS CREDIT",
        amount: 164.16,
      });
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
    });
    ...
    it("should ignore trailing text after the amount", () => {
      const raw =
        "04/21/26    BANK REWARDS CREDIT    $164.16     here is some example text.";
      const [row] = parseTransactionPaste(raw);
...
04/21/26    BANK REWARDS CREDIT    $164.16`;
    ...
    it("should not confuse Format G with Format B (double-date)", () => {
      ...
      expect(rows).toHaveLength(2);
      expect(rows[0]).toMatchObject({
        description: "SINGLE DATE MERCHANT",
        amount: 21.76,
      });
      expect(rows[1]).toMatchObject({
        description: "DOUBLE DATE MERCHANT",
        amount: 50,
      });
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
    });
  ...
  describe("Format H — Target pending transaction", () => {
    ...
    it("should parse multiple consecutive Format H blocks", () => {
      ...
      expect(rows).toHaveLength(2);
      expect(rows[0]).toMatchObject({
        description: "TGT.COM 912003433044748",
        amount: 54.97,
      });
      expect(rows[1]).toMatchObject({
        description: "TGT PLUS 912003433044749",
        amount: 86.67,
      });
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
    });
...
TARGET 1144 COON RAPIDS MN    Sale    **3370    $59.83`;
      ...
      expect(rows[2]).toMatchObject({
        description: "E-PAYMENT - THANK YOU *",
        amount: -348.0,
      });
      expect(rows[3]).toMatchObject({
  ...
  describe("Format I — weekday+date section header with ACH deposit lines", () => {
    ...
    it("should parse multiple consecutive Format I blocks", () => {
      const raw = [
        ...
        blockI(
          "Tuesday",
          "Apr 28",
          "Deposit ACH PAYCO TYPE: PAYROLL ID: 9000000013 CO: ACME CORPORATION Entry Class Code: PPD ACH Trace Number: 021000000000085",
          "$54.91",
        ),
        blockI(
          "Tuesday",
          "Apr 28",
          "Deposit ACH PAYCO TYPE: PAYROLL ID: 9000000013 CO: ACME CORPORATION Entry Class Code: PPD ACH Trace Number: 021000000000086",
          "$125.00",
        ),
      ].join("\n");
      const rows = parseTransactionPaste(raw);
    ...
    it("should fall back to full description when no CO: field is present", () => {
      const raw = blockI(
        "Monday",
        "Mar 10",
        "Deposit Wire Transfer ACME CORP",
        "$500.00",
      );
      const [row] = parseTransactionPaste(raw);
    ...
    it("should parse multiple consecutive Format J blocks", () => {
      const raw = [
        ...
        blockJ(
          "May 10",
          "1% Cash Back",
          "GENERAL STORE 1234 SPRINGFIELD",
          "John Q Smith",
          "$23.99",
        ),
      ].join("\n");
      ...
      expect(rows).toHaveLength(2);
      expect(rows[0]).toMatchObject({
        description: "ACME GROCERY #8 SPRINGFIELD",
        amount: 46.37,
      });
      expect(rows[1]).toMatchObject({
        description: "GENERAL STORE 1234 SPRINGFIELD",
        amount: 23.99,
      });
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
    });
...
$50.80`;
  ...
  describe("mixed formats in one paste", () => {
    ...
    it("should parse all seven formats in the same paste", () => {
      const raw = [
        ...
        blockG(
          "04/24/26",
          "ONLINE TRANSFER FROM CHECKING REF #YYYYYYYYYY WAY2SAVE SAVINGS",
          "$10.88",
        ),
      ].join("\n");
  ...
  describe("trailing US state stripping", () => {
    ...
    it("should strip different state codes", () => {
      ...
      for (const st of states) {
        const [row] = parseTransactionPaste(
          blockA(1, "04/16/26", `SOME MERCHANT ${st}`, "#...9999", "$10.00"),
        );
    ...
    it("should not strip a non-state two-letter suffix", () => {
      const [row] = parseTransactionPaste(
        blockA(1, "04/16/26", "MERCHANT XX", "#...9999", "$10.00"),
      );
    ...
    it("should not strip a state abbreviation that is part of the merchant name", () => {
      const [row] = parseTransactionPaste(
        blockA(
          1,
          "04/16/26",
          "PAYPAL *GLOBALPAYINC 800-555-1234 INTL",
          "#...9999",
          "$4.95",
        ),
      );
  ...
  describe("amount parsing", () => {
    ...
    it("should parse a negative amount with leading minus before $", () => {
      const [row] = parseTransactionPaste(
        blockA(1, "04/16/26", "REFUND", "#...9999", "-$12.00"),
      );
    ...
    it("should parse amounts with comma thousands separators", () => {
      const [row] = parseTransactionPaste(
        blockA(1, "04/16/26", "BIG PURCHASE", "#...9999", "$1,234.56"),
      );
    ...
    it("should parse a zero amount", () => {
      const [row] = parseTransactionPaste(
        blockA(1, "04/16/26", "FREE ITEM", "#...9999", "$0.00"),
      );
    ...
    it("should ignore running balance and capture only the first amount", () => {
      const [row] = parseTransactionPaste(
        blockB("04/12/26", "BRAKES PLUS", "#REF", "$36.45    $12,204.24"),
      );
  ...
  describe("error / ambiguous rows", () => {
    it("should flag a row when no amount line is found", () => {
      const raw =
        "Transaction Details for Row 1    04/16/26    FOOD MART 55555\n#...9999";
      const [row] = parseTransactionPaste(raw);
      expect(row.amount).toBeNull();
      expect(
        row.parseErrors.some((e) => e.toLowerCase().includes("amount")),
...
$29.73`;
      ...
      expect(rows[3]).toMatchObject({
        description: "BURGER JOINT 'S F99999",
        amount: 1.07,
      });
...
$36.45    $12,204.24`;
      ...
      rows.forEach((r) => expect(r.parseErrors).toHaveLength(0));
      expect(rows[0]).toMatchObject({
        description: "WEBPAY*LAKEWOOD-DISTRICT 555-867-5309 XX",
        amount: 50,
      });
      expect(rows[1]).toMatchObject({
...
Show Transaction`;
      ...
      expect(rows[5]).toMatchObject({
        description: "GENERAL STORE 011445SPRINGFIELD XX",
        amount: 5.4,
      });
...
04/20/26    ONLINE TRANSFER REF #ZZZZZZZZZZ TO VISA CARD XXXXXXXXXXXX1111 ON 04/18/26        $169.00`;
      ...
      expect(rows[6]).toMatchObject({
        description: "ONLINE TRANSFER",
        notes: "REF #ZZZZZZZZZZ TO VISA CARD XXXXXXXXXXXX1111 ON 04/18/26",
        amount: 169.0,
      });
      expect(rows[0].date!.getMonth()).toBe(3); // April
      expect(rows[0].date!.getDate()).toBe(27);
