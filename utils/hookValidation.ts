 ...
 * @example
 * ```typescript
 * const validatedAccount = validateInsert(
 *   accountData,
 ...
 * ```
 */
export function validateInsert<T, R = T>(
  data: T,
 ...
 * ```
 */
export function validateUpdate<T, R = T>(
  newData: T,
  validator: (data: T) => ValidationResult<R>,
  operationName: string,
): R {
 ...
 * ```
 */
export function validateDelete<T>(
  data: T,
 ...
 * @example
 * ```typescript
 * const validGuid = validateGuid(
 *   transaction.guid,
 *   "updateTransaction"
 * );
 * ```
 */
export function validateGuid(guid: string, operationName: string): string {
  const guidRegex =
 ...
 * @example
 * ```typescript
 * const validId = validateNumericId(
 *   account.accountId,
 *   "accountId",
 *   "updateAccount"
 * );
...
export function withValidation<T, R = T>(
  ...
  return function (data: T): R {
    return validateInsert(data, validator, operationName);
  };
}
