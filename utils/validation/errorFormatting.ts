 ...
 */
export function createFieldErrorMap(errors: ValidationError[]): Record<string, string> {
 ...
 */
export function getErrorSummary(errors: ValidationError[], maxErrors: number = 3): string {
  if (errors.length === 0) {
    return 'Validation failed';
  }
 ...
 */
export function getErrorSeverity(error: ValidationError): 'error' | 'warning' | 'info' {
  // Check error code for severity hints
  const warningCodes = ['SUSPICIOUS_AMOUNT', 'UNUSUAL_DATE', 'POTENTIAL_DUPLICATE'];
